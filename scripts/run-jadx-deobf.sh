#!/usr/bin/env bash
# JADX run: Temurin 25, ParallelGC, 24GB heap, 12 threads, deobf ON, show-bad-code OFF.
...
OUT_DEC="$ROOT/decode/jadx-deobf-normal"
...
ts="$(date +%Y%m%d-%H%M%S)"
log="$OUT_LOG/jadx_deobf_normal_${THREADS}_${ts}.log"

cmd=(jadx --threads-count 12 --deobf --no-debug-info -d "$OUT_DEC" "$APK")

echo "==> Temurin25 ParallelGC  heap=24GB  threads=12  timeout=${TIMEOUT_S}s  JAVA_HOME=$JAVA_HOME"
echo "==> DEOBF: ON  SHOW-BAD-CODE: OFF"
echo "Command: ${cmd[*]}"
echo "Logs: $log"
echo "-----------------------------------------------"
